...
def monte_carlo_simulation(starting_amount, spending, deposit, w_years, r_years, w1, plot=False):
    ...
    for i in range(1, years):
        ...
        if i <= w_years:
            price_series[i] = max(price_series[i] + deposit, 0)
        else:
            withdrawal = calculate_withdrawal_for_spending(spending, 0, 0, 'married', calculate_federal_taxes)
...
def run_monte_carlo_simulation(starting_amount, spending, deposit, w_years, r_years, stock_weight, num_simulations=100):

    pass_count = 0
    fail_count = 0
    ending_value_sum = 0
    for i in range(num_simulations):
        price_series = monte_carlo_simulation(starting_amount, spending, deposit, w_years, r_years, stock_weight)
        if price_series[-1] > 0:
            pass_count += 1
    ...
    ending_value = ending_value_sum / num_simulations
